...
//for btn setting
void D2dInforDataSingle::setKeyValue(QString KeyName, int value)
{
    QSettings settings;
    settings.beginGroup("BTNSETTING");
    settings.setValue(KeyName, value);
    settings.endGroup();
}

int D2dInforDataSingle::getKeyValue(QString KeyName)
{
    QSettings settings;
    settings.beginGroup("BTNSETTING");
    int tempValue = settings.value(KeyName).toInt();
    settings.endGroup();
    return tempValue;
}

void D2dInforDataSingle::clearChSetting(int chValue)
{
    QSettings settings;
    settings.beginGroup("BTNSETTING");
    ...
    {
        //qCritical() << "D2dInforDataSingle clearChSetting ." << keys.at(i);
        int tempValue = settings.value(keys.at(i)).toInt();
        if(tempValue == chValue)
        {
            settings.setValue(keys.at(i), 0);
        }
    }
    settings.endGroup();
}

QString D2dInforDataSingle::getChModelStr(QString KeyName)
{
    QString temStr;
    if(getKeyValue(KeyName) == 0)
    {
        temStr = "Loosen to reset";
    }
    else
    {
        temStr = "Loosen to keep";
    }
    return temStr;
}

QString D2dInforDataSingle::getChStr(int chValue)
{
    QString temStr;
    QSettings settings;
    settings.beginGroup("BTNSETTING");
    ...
    for (int i = 0; i < keys.count(); ++i)
    {
        ...
        if(tempValue == chValue)
        {
            chBtnListQStr.append(getStrFromKey(keys.at(i)));
            if(!temStr.isEmpty())
                temStr += ",";
            temStr += getStrFromKey(keys.at(i));
        }
    }
    settings.endGroup();
    return temStr;
}
int D2dInforDataSingle::getComboxList(int index)
{
    if(index > chBtnListQStr.count())
        return 0;
    return getIndexFromStr(chBtnListQStr.at(index - 1));
}

int D2dInforDataSingle::getIndexFromStr(QString tmpStr)
{
    int  returnValue = 0;
    if(tmpStr == "A long")
    {
    ...
    else if(tmpStr == "A short")
    {
        returnValue = 1;
    }
    else if(tmpStr == "B long")
    {
        returnValue = 2;
    }
    else if(tmpStr == "B short")
    {
        returnValue = 3;
    }
    else if(tmpStr == "C long")
    {
        returnValue = 4;
    }
    else if(tmpStr == "C short")
    {
        returnValue = 5;
    }
    else if(tmpStr == "D long")
    {
        returnValue = 6;
    }
    else if(tmpStr == "D short")
    {
        returnValue = 7;
    }
    return returnValue;;
...
QString D2dInforDataSingle::getStrFromKey(QString tmpStr)
{
    QString returnStr;
    if(tmpStr == "AL")
    {
...
QString  D2dInforDataSingle::getKeyNameFromCombox(int index)
{
    QString temStr;
    switch (index)
    {
    ...
    //qCritical() << "D2dInforDataSingle getKeyNameFromCombox ." << temStr;
    return temStr;
}
